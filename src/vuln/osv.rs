...
fn parse_severity(val: &serde_json::Value) -> Option<Severity> {
    ...
    // Try to extract numeric score from CVSS vector or direct score field.
    let score = val
        .get("score")
        .and_then(|v| v.as_f64())
        .or_else(|| extract_cvss_base_score(score_str?));
...
/// Attempt to extract the base score from a CVSS vector string.
/// CVSS:3.1/AV:N/AC:L/... doesn't contain a numeric score directly,
/// but some OSV entries use the score field as a string number.
